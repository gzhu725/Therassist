...
app.get("/clients/getData/:clientName", async (req, res) => {
  ...
  //
  try {
    const client = await ClientModel.findOne({ name: client_name }).select(
      "sessions"
    );
    if (!client) {
      return res
...
app.get("/clients/getAllData/:clientName", async (req, res) => {
  const client_name = req.params.clientName;
  //
  try {
...
app.get("/clients/getSummary/:clientName", async (req, res) => {
  const client_name = req.params.clientName;
  try {
    const client_summary = await ClientModel.findOne({
      name: client_name,
    }).select("summary -_id");
    if (!client_summary) {
      return res
        .status(404)
        .json({ success: false, message: "Client summary not found" });
    }
